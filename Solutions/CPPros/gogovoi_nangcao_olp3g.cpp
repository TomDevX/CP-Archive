 ...
 *    note: Notice that sum(i: 1 -> n){sum(j: 1 -> n){max(i,j) - min(i,j)}} = sum(i: 1 -> n){sum(j: 1 -> n){max(i,j)}} - sum(i: 1 -> n){sum(j: 1 -> n){min(i,j)}}. Now we have 2 separate problems. For the max problem, We sort w[i] increasing. Now imagine that we are in a graph, a union of nodes are nodes that are connected together and there will be a max value in that group. For example, we are processing node X, which has the current highest value (in already processed nodes) so far, now X is in union A and has an edge to a node Y with lower value from union B, since Y is in union B, Y also connects to all nodes in B, so when we connect X to Y, all nodes in B must go through X, and since X has current the highest value, our total cost to go from a node from A to B (or B to A) is size[A]*size[B]*w[X]. From this example, we continue doing it with the higher values ahead. To manage the unions, we use DSU. And then we do the same with sum of mins to finish the final given formula
**/
