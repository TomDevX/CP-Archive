...
# Tự động lấy thư mục nơi file script đang đứng làm gốc
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
README_FILE = os.path.join(BASE_DIR, 'README.md')
# Với bản con, root_dir chính là thư mục hiện tại
root_dir = BASE_DIR
...
def format_display_name(name, is_root=False):
    if not name: return ""
    # Nếu là thư mục gốc của folder con này, lấy tên thư mục đó làm tiêu đề
    parts = name.split('_')
...
def generate_readme():
    ...
    for path, files in folder_data:
        ...
        # Nếu là thư mục gốc của script này
        if rel_path_local == ".":
        ...
        for i, file in enumerate(files, 1):
            ...
            # Link Solution tương đối so với file README hiện tại
    ...
    stats += f"- **Problems in this category:** {total_problems_count}\n"
    stats += f"- **Solved:** {total_ac}\n"
