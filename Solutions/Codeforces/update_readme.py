...
def run_sub_scripts():
    ...
    print("🔍 Đang tìm kiếm script con...")
    ...
    for root, dirs, files in os.walk(BASE_DIR):
        ...
        if "update_readme.py" in files:
            ...
            if script_path != current_script_path:
                ...
                try:
                    # Chạy script với working directory là thư mục chứa script đó
                    subprocess.run(["python", script_path], cwd=root, check=True)
                except subprocess.CalledProcessError as e:
                    print(f"❌ Lỗi khi chạy {script_path}: {e}")
                except Exception as e:
                    print(f"⚠️ Lỗi không xác định tại {script_path}: {e}")

def generate_readme():
    # Thực hiện chạy các script con trước
    run_sub_scripts()
    
    print("📝 Đang khởi tạo nội dung README tổng...")
