...
namespace sub2{
    ...
    void del_seg(int id, int l, int r, int pos, int val){
        if(l == r){
            multiset<int>::iterator it = leaf[l].find(val);
            if(it == leaf[l].end()){
                st[id] = st[0];
                return;
            }
            leaf[l].erase(it);
            st[id] = (leaf[l].empty() ? st[0] : *leaf[l].rbegin());
            return;
        }
        ...
        if(pos <= mid) del_seg(left,l,mid,pos,val);
        else del_seg(left|1,mid+1,r,pos,val);
        st[id] = max(st[left], st[left|1]);
    }
    ...
    void solve(){
        ...
        vi sorted = prefD;
        sort(all(sorted,0));
        for(int i = 0; i <= n; i++) prefD[i] = lower_bound(all(sorted,0), prefD[i]) - sorted.begin();
        ...
        // for(int i = liml + 1; i <= n; i++){
        //     if(i > limr) del_seg(1,0,n,prefD[i-limr-1], dp[i-limr-1]);
        //     add_seg(1,0,n,prefD[i-liml],dp[i-liml]);

        //     dp[i] = max((int)-1e9,max({get(1,0,n,0,prefD[i]-1) + 1, get(1,0,n,prefD[i],prefD[i]), get(1,0,n,prefD[i]+1,n) - 1}));
        // }

        for (int i = 1; i <= n; i++) {
            // 1. Nạp dp[k] khi k = i - liml lọt vào cửa sổ
            if (i >= liml) {
                int k = i - liml;
                add_seg(1, 0, n, prefD[k], dp[k]);
            }

            // 2. Xóa dp[k] khi k = i - limr - 1 trượt ra ngoài
            if (i > limr) {
                int k = i - limr - 1;
                del_seg(1, 0, n, prefD[k], dp[k]);
            }

            // 3. Truy vấn 3 vùng trên SegTree
            int cur_id = prefD[i];
            int res = -1e9;

            int q_less = get(1, 0, n, 0, cur_id - 1);
            if (q_less > -1e9) res = max(res, q_less + 1);

            int q_eq = get(1, 0, n, cur_id, cur_id);
            if (q_eq > -1e9) res = max(res, q_eq);

            int q_more = get(1, 0, n, cur_id + 1, n);
            if (q_more > -1e9) res = max(res, q_more - 1);
